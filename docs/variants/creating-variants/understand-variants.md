...
> **Note:** A variant and a plugin can come from the same dependency, but for the clarity of the documentation we keep them separate. To find out what's possible from a plugin, see the [Plugins documentation](/latest/plugins).
...
Variants are published as NPM packages. For details, see [Publish a variant](/latest/variants/creating-variants/publish-a-variant).
